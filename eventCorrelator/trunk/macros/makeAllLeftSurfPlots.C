...
void makeAllLeftSurfPlots() 
{
   ...
   deltaTTree->Add("deltaTFilesFastClock*.root");
   ...
   for(int surf=0;surf<8;surf++) {
	 ...
	 histSurfDiff[surf][chip]=new TH1F(histName,plotTitle,200,-3,3);
      }
   }
   ...
   for(int ant=0;ant<32;ant++) {
	 ...
	 TH1F *histDt11 = new TH1F(histName,plotTitle,200,-3,3);
   ...
   for(int ant=0;ant<16;ant++) {     
	 ...
	 sprintf(plotTitle,"Ant %d -  Ant %d (Chip %d)",leftDownAnt,middleAnt,chip);
	 sprintf(histName,"ldhistDt_%d_%d",ant,chip);
	 TH1F *histDt11 = new TH1F(histName,plotTitle,200,-3,3);
	 deltaTTree->Project(histName,"-1*(deltaT-deltaTExpected)",plotCond);

	 histSurfDiff[leftSurf][chip]->Add(histDt11);
      }
   }
   ...
   for(int ant=0;ant<16;ant++) {     
      ...
      fGeomTool->getThetaPartners(middleAnt,leftAnt,rightAnt); 
      //      cout << leftAnt << "\t" << middleAnt << "\t" << rightAnt << "\n";
      int rightDownAnt=fGeomTool->getAzimuthPartner(rightAnt);
      //      cout << rightDownAnt << "\n";
      Int_t ci=fGeomTool->getChanIndexFromAntPol(middleAnt,AnitaPol::kVertical);
      Int_t middleSurf=ci/9;


      for(int chip=0;chip<4;chip++) {	 
	 sprintf(plotCond,"((firstAnt==%d && secondAnt==%d))  && labChip==%d && (corPeak/corRMS)>6",middleAnt,rightDownAnt,chip);	 
	 //	 sprintf(plotTitle,"Ant %d -  Ant %d (Chip %d)",rightDownAnt,middleAnt,chip);
	 sprintf(plotTitle,"Ant %d -  Ant %d (Chip %d)",middleAnt,rightDownAnt,chip);
	 sprintf(histName,"rdhistDt_%d_%d",ant,chip);
	 TH1F *histDt11 = new TH1F(histName,plotTitle,200,-3,3);

	 deltaTTree->Project(histName,"deltaT-deltaTExpected",plotCond);
	 histSurfDiff[middleSurf][chip]->Add(histDt11);
   ...
   for(int surf=0;surf<8;surf++) {
      canSurf->cd(surf+1);
      for(int chip=0;chip<4;chip++) {
	 fitty->SetRange(histSurfDiff[surf][chip]->GetMean()-0.5,histSurfDiff[surf][chip]->GetMean()+0.5);
