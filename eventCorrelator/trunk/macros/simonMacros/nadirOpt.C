...
double nadirOpt(double *par){
  ...
  for(int i = 32; i<40;i++){
    deltaR[i]=par[i-32];
    //deltaZ[i]=par[i+16];
    deltaPhi[i]=par[i-32+8];
    deltaTArrayMod[i] = par[i-32+16];

 cout << i << "  " << deltaTArrayMod[i] << " " << deltaR[i] << " " << deltaPhi[i] << " " << 0 << endl;
...
double sumGrads = 0;
  ...
  for(int loop = 1; loop <4; loop++){
	...
	if((deltaT - deltaTExpected)*(deltaT - deltaTExpected) < 1 && (corPeak/corRMS)>9.5 ){
  ...
  for(int ants = 32; ants < 40; ants++){
	...
	if(ants == 8 || ants == 16 || ants == 12 || ants == 39){
	  tempAntGraph->SetMarkerColor(8);
	}

	if(ants == 3 || ants == 7 || ants == 23 || ants == 32){
	  tempAntGraph->SetMarkerColor(1);
	}
    
	if(ants == 9 || ants == 13 || ants == 18 || ants == 33){
	  tempAntGraph->SetMarkerColor(2);
	}
    
    
	if(ants == 10 || ants == 14 || ants == 20 || ants == 34){
	  tempAntGraph->SetMarkerColor(3);
	}
    
	if(ants == 11 || ants == 15 || ants == 22 || ants == 35){
	  tempAntGraph->SetMarkerColor(4);
	}
    
	if(ants == 0 || ants == 4 || ants == 17 || ants == 36){
	  tempAntGraph->SetMarkerColor(5);
	}


	if(ants == 2 || ants == 6 || ants == 21 || ants == 37){
	  tempAntGraph->SetMarkerColor(6);
	}

	if(ants == 1 || ants == 5 || ants == 19 || ants == 38){
...
//     deltaR[i]=par[i-32];
//     //  deltaZ[i]=par[i-32+8];
//     deltaPhi[i]=par[i-32+16];
//     deltaTArrayMod[i] = par[i-32+8];
//   }
...
// 		//	tempLine = new TGraph(2, tempY, tempX);

// 		//		cout << slope << "  " << intercept << "  " << tempX[0] << "  " << tempY[0] << "  " << tempX[1] << "   " << tempY[1] << endl;
...
// // 	tempLine->SetMinimum(-2.5);
// // 	tempLine->SetMaximum(1.5);
	
// // 	tempLine->SetMarkerStyle(1);
   
// // 	tempLine->GetXaxis()->SetLimits(0,360);
// // 	//	myMG3->Add(tempLine);
// // 	//	myMG3->Draw("p");  
...
//   double theReturn =  sumGrads + sumMean2;
