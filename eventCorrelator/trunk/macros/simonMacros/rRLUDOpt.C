...
double rRLUDOpt(double *par);  
double rRLUDOpt(double *par){
  
  
  double deltaRIn = par[0];
  double deltaRL = par[1];
  double deltaUD = par[2];
...
//   deltaTArrayMod[rightOpt] = par[4]; 
  ...
  for(entry=0;entry<numEntries;entry++) {
    ...
    pitch=pat->pitch;
    roll=pat->roll;
    UsefulAdu5Pat usefulPat(pat,deltaRIn,deltaRL,deltaUD);
  ...
  cout << "r "<< par[0]<< " RL "<< par[1] << " UD " << par[2] << " sum mean " << sumMean << " sumRMS " << sumRMS  << endl;
