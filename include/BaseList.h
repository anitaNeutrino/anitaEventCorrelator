...
namespace BaseList {
  ...
  /** Return the ith path waypoint as base . This function knows about the ANITA version */ 
  const base & getPathAsBase(UInt_t i);

  /** Return the ith base or path. This function knows about the ANITA version */ 
  const abstract_base & getAbstractBase(UInt_t i, bool asBases = false); //  both 
  ...
  /** Returns the number of flights/traverses as bases. ANITA-version aware */
  size_t getNumPathsAsBases();

  /** Returns the number of flights/ traverses + bases. ANITA-version aware */
  size_t getNumAbstractBases(bool asBases = false); 
   ...
   *
   **/ 
  int findBases(const char * query, std::vector<int> * all_matches = 0, bool include_paths = false, bool asBases = false);
};
