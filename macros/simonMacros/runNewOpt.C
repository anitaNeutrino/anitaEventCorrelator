...
void runNewOpt() {
  ...
  gSystem->CompileMacro("newOpt.C","k");
  
  Double_t relDeltaOut=0;

  TMinuit *myMin = new TMinuit(150);
  myMin->SetObjectFit(newOpt);
...
for(int y = 0; y <32; y++){
   ...
   myMin->DefineParameter(y+64, name, startValPhi[y], stepSize, minVal, maxVal);
   sprintf(name,"dt%d",y);
   myMin->DefineParameter(y+96, name, startValPhi[y], stepSize, minVal, maxVal);
...
//  }
	...
	for(int u = 0; u <32; u++){
	  ...
	  myMin->GetParameter(u+96,deltaT[u],deltaTErr[u]);
