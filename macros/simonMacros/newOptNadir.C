...
double newOptNadir(double *par){
  ...
  for(int i = 0; i<8;i++){
    deltaR[i+32]=par[i];   
    deltaZ[i+32]=par[i+8];
    deltaPhi[i+32]=par[i+16];
    deltaTArrayMod[i+32]=par[i+32];
    cout << i+32 << "  " <<deltaTArrayMod[i+32] << " " << deltaR[i+32] << " " << deltaPhi[i+32] << " " << deltaZ[i+32] << endl;
