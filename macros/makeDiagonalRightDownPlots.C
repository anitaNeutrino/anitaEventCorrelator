...
void makeDiagonalRightDownPlots() 
{
   ...
   ofstream Output("diffsDiagonalRightDown.txt");
   ...
   for(int ant=0;ant<16;ant++) {     
	 ...
	 can->cd((4*ant)+chip+1);
	 //	 sprintf(plotCond,"((firstAnt==%d && secondAnt==%d))  && labChip==%d && (corPeak/corRMS)>6",rightDownAnt,middleAnt,chip);	 
...
// 	      << fitty->GetParError(2)  << "\t" << fitty->GetChisquare() << "\t" << fitty->GetNDF() << "\n";
	 ...
	 Output << middleAnt << "\t" << rightDownAnt << "\t" << chip << "\t" << constant << "\t" << error << "\t"
		<< entries << "\n";
